...
// ✅ CORREÇÃO: Contar apenas estrelas da FASE 1
$total_acertos = contarEstrelasFase($pdo, $usuario_id, 1);

// DEBUG
error_log("DEBUG FINAL: Usuário $nomeAluno tem $total_acertos estrelas na FASE 1");

// DEBUG: Ver todas as estrelas do usuário
$stmt = $pdo->prepare("SELECT * FROM estrelas WHERE nomeAluno = ?");
$stmt->execute([$nomeAluno]);
$todas_estrelas = $stmt->fetchAll();
error_log("DEBUG: Total de registros no banco = " . count($todas_estrelas));
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
